...
downlaod_cool_roof_tree_data <- function(city, aoi_name, scenario, baseline_folder, tiles) {
  ...
  scenario_folder <- glue("city_projects/{city}/{aoi_name}/scenarios/cool-roofs_trees/{scenario}")
  
  tree_scenario <- combo |> 
    filter(infra == "trees") |> 
    pull(scenario)
  
  tree_scenario_folder <- glue("city_projects/{city}/{aoi_name}/scenarios/trees/{tree_scenario}")
  
  cool_roof_scenario <- combo |> 
    filter(infra == "cool-roofs") |> 
    pull(scenario)
  
  cool_roof_scenario_folder <- glue("city_projects/{city}/{aoi_name}/scenarios/cool-roofs/{cool_roof_scenario}")
  ...
  # Create setup folder for new run
  run_setup_folder <- file.path(ctcm_setup_path, glue("{city}_cool-roofs_trees_{scenario}"))
  ...
  for (t in tiles){
    ...
    baseline_tile <- glue("{baseline_folder}/{t}")
    scenario_tile <- glue("{scenario_folder}/{t}")
    tree_scenario_tile <- glue("{tree_scenario_folder}/{t}")
    ...
    base_files <- c(
      ...
      # scenario layers
      glue("{scenario_tile}/ccl_layers/albedo__cool-roofs_trees__{scenario}.tif"),
      glue("{tree_scenario_tile}/raster_files/tree_canopy.tif")
    ...
    svf_files  <- c("svf.tif","svfaveg.tif","svfE.tif","svfEaveg.tif","svfEveg.tif",
                    "svfN.tif","svfNaveg.tif","svfNveg.tif","svfS.tif","svfSaveg.tif",
                    "svfSveg.tif","svfveg.tif","svfW.tif","svfWaveg.tif","svfWveg.tif")
    svf_files  <- glue("{tree_scenario_tile}/processed_data/ctcm_svfs/{svf_files}")
  ...
  met_file <- glue("{cool_roof_scenario_folder}/metadata/met_files/reduced_temps.csv")
  download_s3_files(
    bucket    = "wri-cities-tcm",
    s3_keys   = met_file,
    local_dir = glue("{run_setup_folder}/primary_data/met_files")
...
run_cool_roof_tree_CTCM <- function(city, infra, scenario, aoi_name, aoi){
  ...
  # CustomTiffFilenames
  scenario_yaml[[4]]$albedo_cloud_masked_tif_filename <- glue("albedo__cool-roofs_trees__{scenario}.tif")
  scenario_yaml[[4]]$dem_tif_filename <- "cif_dem.tif"
  scenario_yaml[[4]]$dsm_tif_filename <- "cif_dsm_ground_build.tif"
  scenario_yaml[[4]]$lulc_tif_filename <- "cif_lulc.tif"
  scenario_yaml[[4]]$open_urban_tif_filename <- "cif_open_urban.tif"
