...
calc_baseline_metrics <- function(city, aoi_name, tiles_aoi) {
  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        ...
        glue("pct_shade_cover_{time}_baseline_aoi"),
        glue("mean_distance_shade_cover_{time}_baseline_aoi"),
        glue("pct_high_risk_utci_{time}_baseline_aoi"),
        
        glue("mean_utci_{time}_baseline_pedestrian_area"),
        glue("pct_shade_cover_{time}_baseline_pedestrian_area"),
        glue("mean_distance_shade_cover_{time}_baseline_pedestrian_area"),
        glue("pct_high_risk_utci_{time}_baseline_pedestrian_area"),
        
        glue("mean_utci_{time}_baseline_nonbuilding_area"),
        glue("pct_shade_cover_{time}_baseline_nonbuilding_area"),
        glue("mean_distance_shade_cover_{time}_baseline_nonbuilding_area"),
        glue("pct_high_risk_utci_{time}_baseline_nonbuilding_area")
      ),
      value = c(
  ...
  tree_metrics <- tibble(
    indicators_id = c(
      "pct_tree_cover_baseline_aoi",
      "pct_tree_cover_baseline_pedestrian_area",
      "n_tree_baseline_aoi",
      "n_tree_baseline_pedestrian_area",
      "area_pedestrian",
      "area_nonbuilding",
      "area_parks"
    ),
...
calc_street_tree_metrics <- function(city, aoi_name, tiles_aoi, infra, scenario) {
  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        ...
        glue("pct_shade_cover_{time}_baseline_aoi"),
        glue("pct_shade_cover_{time}_scenario_aoi"),
        glue("pct_shade_cover_{time}_change_aoi"),
        glue("mean_distance_shade_cover_{time}_baseline_aoi"),
        glue("mean_distance_shade_cover_{time}_scenario_aoi"),
        glue("mean_distance_shade_cover_{time}_change_aoi"),
        
        glue("mean_utci_{time}_baseline_pedestrian_area"),
        glue("mean_utci_{time}_scenario_pedestrian_area"),
        glue("mean_utci_{time}_change_pedestrian_area"),
        glue("pct_shade_cover_{time}_baseline_pedestrian_area"),
        glue("pct_shade_cover_{time}_scenario_pedestrian_area"),
        glue("pct_shade_cover_{time}_change_pedestrian_area"),
        glue("mean_distance_shade_cover_{time}_baseline_pedestrian_area"),
        glue("mean_distance_shade_cover_{time}_scenario_pedestrian_area"),
        glue("mean_distance_shade_cover_{time}_change_pedestrian_area"),
        glue("pct_high_risk_utci_{time}_baseline_pedestrian_area"),
        glue("pct_high_risk_utci_{time}_scenario_pedestrian_area"),
        glue("pct_high_risk_utci_{time}_change_pedestrian_area"),
        
        glue("mean_utci_{time}_baseline_nonbuilding_area"),
        glue("mean_utci_{time}_scenario_nonbuilding_area"),
        glue("mean_utci_{time}_change_nonbuilding_area"),
        glue("pct_shade_cover_{time}_baseline_nonbuilding_area"),
        glue("pct_shade_cover_{time}_scenario_nonbuilding_area"),
        glue("pct_shade_cover_{time}_change_nonbuilding_area"),
        glue("mean_distance_shade_cover_{time}_baseline_nonbuilding_area"),
        glue("mean_distance_shade_cover_{time}_scenario_nonbuilding_area"),
        glue("mean_distance_shade_cover_{time}_change_nonbuilding_area"),
        
        glue("max_utci_{time}_change_impact_area"),
        glue("mean_utci_{time}_change_impact_area"),
        glue("pct_utci_impact_{time}_aoi")
      ),
      value = c(
        mean_utci_bl_aoi,
        mean_utci_sc_aoi,
  ...
  tree_metrics <- tibble(
    indicators_id = c(
      "pct_tree_cover_baseline_aoi",
      "pct_tree_cover_scenario_aoi",
      "pct_tree_cover_change_aoi",
      "pct_tree_cover_baseline_pedestrian_area",
      "pct_tree_cover_scenario_pedestrian_area",
      "pct_tree_cover_change_pedestrian_area",
      "n_tree_baseline_pedestrian_area",
      "n_tree_change_pedestrian_area",
      "n_tree_scenario_pedestrian_area",
      "pct_tree_cover_achievable_pedestrian_area",
      "tree_cover_progress_pedestrian_area",
      "mean_air_temp_1500_baseline_aoi",
      # "mean_air_temp_1500_scenario_aoi",
      # "mean_air_temp_1500_change_aoi",
      "mean_air_temp_1500_change_aoi",
      "pct_air_temp_impact_1500_aoi",
      "area_pedestrian"
    ),
    value = c(
...
calc_cool_roofs_metrics <- function(city, aoi_name, tiles_aoi, infra, scenario) {
  ...
  scalar_metrics <- tibble(
    indicators_id = c(
      "cool_roof_reflectivity_achievable",
      "cool_roof_reflectivity_high_achievable",
      "cool_roof_reflectivity_low_achievable",
      "mean_reflectivity_achievable_aoi",
      "pct_air_temp_impact_1500_aoi",
      "area_cool_roof_baseline",
      "mean_air_temp_1200_baseline_aoi",
      "mean_air_temp_1500_baseline_aoi",
      "mean_air_temp_1800_baseline_aoi",
      "mean_reflectivity_baseline_aoi",
      "mean_roof_reflectivity_baseline",
      "area_cool_roof_change",
      "mean_air_temp_1200_change_aoi",
      "mean_air_temp_1500_change_aoi",
      "mean_air_temp_1800_change_aoi",
      "mean_reflectivity_change_aoi",
      "mean_roof_reflectivity_change",
      "cool_roofs_progress_aoi",
      "reflectivity_progress_aoi",
      "area_cool_roof_scenario",
      "mean_air_temp_1200_scenario_aoi",
      "mean_air_temp_1500_scenario_aoi",
      "mean_air_temp_1800_scenario_aoi",
      "mean_reflectivity_scenario_aoi",
      "mean_roof_reflectivity_scenario",
      "area_cool_roof_technical",
      "area_nonbuilding",
      "mean_air_temp_1500_change_impact_area"
    ),
    value = c(
      alb_target_val,
      alb_target_high_val,
  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        ...
        glue("mean_utci_{time}_change_nonbuilding_area"),
        glue("pct_high_risk_utci_{time}_baseline_nonbuilding_area"),
        glue("pct_high_risk_utci_{time}_scenario_nonbuilding_area"),
        glue("pct_high_risk_utci_{time}_change_nonbuilding_area"),
        glue("max_utci_{time}_change_impact_area"),
        glue("mean_utci_{time}_change_impact_area"),
        glue("pct_utci_impact_{time}_aoi")
      ),
...
calc_shade_structures_metrics <- function(city, aoi_name, tiles_aoi, infra, scenario) {
  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        glue("mean_utci_{time}_baseline_parks"),
        glue("mean_utci_{time}_scenario_parks"),
        glue("mean_utci_{time}_change_parks"),
        glue("pct_shade_cover_{time}_baseline_parks"),
        glue("pct_shade_cover_{time}_scenario_parks"),
        glue("pct_shade_cover_{time}_change_parks"),
        ...
        glue("pct_high_risk_utci_{time}_baseline_parks"),
        glue("pct_high_risk_utci_{time}_scenario_parks"),
        glue("pct_high_risk_utci_{time}_change_parks"),
        glue("max_utci_{time}_change_impact_area"),
        glue("mean_utci_{time}_change_impact_area"),
        glue("pct_utci_impact_{time}_aoi")
      ),
  ...
  bind_rows(
    results,
    tibble(
      indicators_id = c("n_shade_structures_change_parks", "area_parks"),
