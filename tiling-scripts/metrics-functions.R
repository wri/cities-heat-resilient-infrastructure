  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        ...
        glue("mean_distance_shade_cover_{time}_baseline_pedestrian_areas"),
...
calc_street_tree_metrics <- function(city, aoi_name, tiles_aoi, infra, scenario) {
  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        ...
        glue("mean_distance_shade_cover_{time}_baseline_pedestrian_areas"),
        glue("mean_distance_shade_cover_{time}_scenario_pedestrian_areas"),
        glue("mean_distance_shade_cover_{time}_change_pedestrian_areas"),
        glue("high_risk_utci_pct_{time}_baseline_pedestrian"),
        ...
        glue("max_utci_{time}_change_aoi"),
        glue("mean_utci_{time}_change_aoi"),
        glue("utci_reduction_pct_{time}_aoi")
      ),
      value = c(
        mean_utci_bl_aoi,
        mean_utci_sc_aoi,
  ...
  tree_metrics <- tibble(
    indicators_id = c(
      ...
      "mean_air_temp_1500_change_aoi",
      "air_temp_reduction_pct_1500_aoi",
      "pedestrian_area"
    ),
    value = c(
...
calc_cool_roofs_metrics <- function(city, aoi_name, tiles_aoi, infra, scenario) {
  ...
  scalar_metrics <- tibble(
    indicators_id = c(
      ...
      "cool_roofs_progress",
      "reflectivity_progress",
      "cool_roof_area_scenario",
...
calc_shade_structures_metrics <- function(city, aoi_name, tiles_aoi, infra, scenario) {
  ...
  for (time in c("1200", "1500", "1800")) {
    ...
    metrics <- tibble(
      indicators_id = c(
        ...
        glue("shade_cover_{time}_baseline_parks"),
        glue("shade_cover_{time}_scenario_parks"),
        glue("shade_cover_{time}_change_parks"),
  ...
  bind_rows(
    results,
    tibble(
      indicators_id = c("shade_structures_n_change", "park_area"),
