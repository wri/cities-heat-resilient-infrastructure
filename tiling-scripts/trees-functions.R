...
process_trees <- function(tree_canopy, city_folder, baseline_folder, tile_id){
  ...
  write_s3(ttops, glue("{bucket}/{baseline_folder}/{tile_id}/layers/tree-points.geojson"))
  write_s3(crowns, glue("{bucket}/{baseline_folder}/{tile_id}/layers/existing-tree-crowns.tif"))
  write_s3(crown_vectors, glue("{bucket}/{baseline_folder}/{tile_id}/layers/existing-tree-crowns.geojson"))
}


# Create pedestrian area --------------------------------------------------

create_pedestrian_area <- function(lulc){
...
create_plantable_area <- function(lulc, pedestrian_area, binary_tree_cover){
...
baseline_processing <- function(tile_id){
  
  ensure_s3_prefix(
    bucket,
    glue("{baseline_folder}/{tile_id}/layers/")
  )
  
  # Load data
  tree_canopy <- rast(glue("{aws_http}/{baseline_folder}/{tile_id}/raster_files/cif_tree_canopy.tif"))
  ...
  write_s3(binary_tree_cover, glue("{bucket}/{baseline_folder}/{tile_id}/layers/tree-cover.tif"))
  
  # Only process trees if there are any nonzero pixels
  # if (global(binary_tree_cover, "max", na.rm = TRUE)[1,1] > 0) {
  #   process_trees(tree_canopy, city_folder, baseline_folder, tile)
  # } else {
  #   return(invisible(NULL))
  # }
  process_trees(tree_canopy, city_folder, baseline_folder, tile_id = tile_id)
  
  # Create pedestrian area
  pedestrian_area <- create_pedestrian_area(lulc)
  write_s3(pedestrian_area, glue("{bucket}/{baseline_folder}/{tile_id}/layers/pedestrian-areas.tif"))
  
  # Create plantable area
  plantable_area <- create_plantable_area(lulc, pedestrian_area, binary_tree_cover)
  
  # dir.create(here(scenario_folder, tile), showWarnings = FALSE, recursive = TRUE)
  ensure_s3_prefix(
    bucket,
    glue("{scenario_folder}/{tile_id}/layers/")
  )
  write_s3(plantable_area, glue("{bucket}/{scenario_folder}/{tile_id}/layers/plantable-areas.tif"))
  ...
  write_s3(available_pts, glue("{bucket}/{scenario_folder}/{tile_id}/layers/plantable-points.geojson"))
}
...
create_tree_population <- function(tiles){
  ...
  tree_paths <- glue("{aws_http}/{baseline_folder}/{tiles}/layers/tree-points.geojson")
  ...
  crown_paths <- paste(aws_http, baseline_folder, tiles, "layers", "existing-tree-crowns.geojson", sep = "/")
...
# aoi <- st_read(here("data", city_folder, "aoi.geojson"))
...
# TODO: Remove this once full grid is available #### 
aoi_grid <- aoi_grid %>% 
  st_filter(buffered_tile_grid) %>% 
  st_filter(tile_grid) 
...
plant_in_gridcell <- function(grid_index, target_coverage, min_dist){
  ...
  plantable_paths <- glue("{aws_http}/{scenario_folder}/{unbuffered_tile_names}/layers/plantable-areas.tif")
  ...
  ped_area_paths <- glue("{aws_http}/{baseline_folder}/{unbuffered_tile_names}/layers/pedestrian-areas.tif")
  ...
  # Current tree cover
  current_tree_cover_paths <- glue("{aws_http}/{baseline_folder}/{unbuffered_tile_names}/layers/tree-cover.tif")
  ...
  while (tree_cover_size < target_tree_cover_size && nrow(available_pts) > 0) {
    ...
    # Rasterize crown
    crown_pixels <- rast(glue("{aws_http}/{baseline_folder}/{crown_geom$tile}/layers/existing-tree-crowns.tif")) %>% 
      crop(crown_geom)
...
elapsed <- Sys.time() - t0
cat(sprintf("Elapsed: %.2f minutes (%.1f seconds)\n",
            as.numeric(elapsed, "mins"),
            as.numeric(elapsed, "secs")))
