...
utci_calc <- function(Ta, ehPa, va, Tmrt) {
  ...
  if (any(is.na(c(Ta, ehPa, va, Tmrt)))) {
    stop("All input parameters must be non-NA values.")
  }
  
  if (!is.numeric(c(Ta, ehPa, va, values(Tmrt)))) {
  ...
  # Return the final UTCI approximation value
  return(UTCI_approx)
}

# Process files
# mrt_files <- list.files(mrt_dir, pattern = "Tmrt", full.names = TRUE)
